...
**数据清洗（在信号提取前必须执行）**：
原始群聊消息中包含大量噪声，必须先清洗再分析：
- **剔除**：机器人定时推送的长篇内容（日报推送、资讯播报、自动通知等）。但保留机器人与群友互动的真实回复、报错和问答
- **剔除**："收到"、"好的"、"OK"、"跟进"等无信息量的应答
- **剔除**：纯表情、纯图片（无文字上下文的）、系统消息（入群/退群通知等）
- **保留**：工作讨论、技术问题、产品/工具分享、链接分享（含评论）、明确的待办/决策

**结构化中间提取（清洗后、信号提取前）**：
从清洗后的有效消息中，先提取以下结构化信息，再用于画像更新：

1. **热点话题**：识别讨论集中度最高的 3-5 个话题，每个提取：
   - 话题名称
   - 一句话摘要
   - 高频关键词（3-5 个高度提纯的核心词，如具体产品名、技术名、项目名——不是"AI"、"技术"这类泛词）
2. **工作信号**：
   - 今日主要工作讨论/进展
   - 遇到的问题与阻碍
   - 明确的待办事项（Action Items）
3. **分享的资源**：群友分享的链接、工具、文档（这些可以补充到 `sources` 中）

**信号提取规则**（基于清洗和结构化后的结果，所有阈值来自 `signal_rules.*`）：
- **话题增强**：热点话题中的关键词命中画像中的 topic → `weight += daily_boost`（上限 1.0）
- **实体信号**：热点话题的关键词中出现公司/产品/市场名 → 新增或更新 `tracking` 条目
- **新话题**：热点话题中出现画像中没有的关键词，且该话题讨论集中度足够高 → 以 `new_topic_initial_weight` 权重新增
- **工作信号转化**：工作讨论中提到的问题/阻碍/待办如果涉及特定产品或技术方向 → 提升对应 topic 权重（团队正在用的东西比泛泛讨论更重要）
- **资源信号**：群友分享的链接如果是一手来源（官方博客/GitHub/产品页），可追加到 `sources.direct`
- **噪声过滤**：仅被单人单次提及且无后续讨论的话题忽略
...
**⚠️ 时效性硬约束**：搜索查询必须携带时间限定词（如"today"、当天日期、"最新"、"2026-03"），确保结果偏向近 24 小时内容。画像决定搜什么方向，但时效性决定搜出来的内容能不能用——没有明确新增事实的旧内容不能进入日报正文，即使它与画像高度相关。
...
**⚠️ 筛选层级（不可颠倒）**：
- **第一层：时效性门槛** — 不是今天/近 24 小时的内容，默认不入选。旧内容只有在今天出现了新的官方发布/定价/benchmark/公测等明确新增事实时才可保留。
- **第二层：画像相关性** — 通过时效性门槛后，再按部门画像的话题权重和跟踪实体排序。
- **画像相关但不够新的内容 = 不入选。** 这是最常见的错误模式，必须主动防范。

**⚠️ 禁止趋势句凑数**：每条日报条目必须对应一个**具体事件**——谁、什么时候、做了什么。"行业正在从 X 转向 Y"、"XX 持续演进"、"竞争正在加速"这类趋势概括**不能作为独立条目入选**。如果搜索结果不足以凑够目标条数，宁可少放几条，也不要用抽象趋势句填充后半部分。趋势判断只属于左栏的"趋势雷达"，不属于资讯卡片。
